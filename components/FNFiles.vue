<template>
    <div>
      <h2>File Search by Name</h2>
  
      <!-- Input for file name search -->
      <input
        v-model="name"
        @input="fetchFiles"
      ...
      <p v-else>No files found with the specified name.</p>
  ...
  export default {
    ...
    methods: {
      async fetchFiles() {
        ...
        try {
          const response = await fetch(`/api/fnsearch?name=${this.name}&page=${this.page}&limit=${this.limit}`);
    ...
    watch: {
      name() {
        this.page = 1; // Reset to the first page when the name changes
        this.fetchFiles();
      },
      limit() {
